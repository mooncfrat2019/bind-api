...
Первый шаг — получить API-ключ. При первом запуске MASTER может создать временный bootstrap-ключ из переменной окружения `BIND_API_BOOTSTRAP_KEY`.

Условия создания bootstrap-ключа:
- таблица `api_keys` пуста;
- переменная `BIND_API_BOOTSTRAP_KEY` задана;
- значение ключа имеет допустимую длину.
...
Пример:
```shell
bash export BIND_API_BOOTSTRAP_KEY="your-very-secure-bootstrap-key-value" ./bind-api
```
После запуска используйте этот ключ для создания постоянного API-ключа через `/api/keys`, затем отзовите bootstrap-ключ.
...
### 8.4. На мастере: получение A-записей зоны
